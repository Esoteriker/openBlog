...
import { MinimalProfile } from "@/components/minimal-profile";
...
const modeLabelsByLocale = {
  en: { simple: "Simple", full: "Full site" },
  zh: { simple: "极简版", full: "完整页" },
  de: { simple: "Kurz", full: "Vollseite" }
};
...
export function PageShell({ children }: PageShellProps) {
  ...
  return (
    <div className={`min-h-screen text-ink ${minimalMode ? "minimal-mode-active" : ""}`}>
      <button
        type="button"
      ...
      >
        {minimalMode ? modeLabels.full : modeLabels.simple}
      </button>

      {minimalMode ? (
        <MinimalProfile locale={locale} profileData={profileData} />
      ) : (
        <>
          <Navbar
            locale={locale}
